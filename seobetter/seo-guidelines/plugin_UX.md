...
- **Loading spinner on inject-fix buttons (v1.5.74+):** when clicked, button shows a CSS spinner animation + "Working..." text. Preserves button width via `minWidth` to prevent layout shift. Spinner uses `@keyframes sb-spin` in `admin.css`. On failure, error reason is shown in a red callout below the button (not just "Retry").
