...
- **Loading animation on inject-fix buttons (v1.5.75+):** when clicked, button shows: (a) translucent progress bar filling from left to right over 30s via `@keyframes sb-progress-fill`, (b) CSS spinner circle, (c) elapsed time counter "Working 0s... 5s... 10s..." updated every second. Button preserves width via `minWidth`. On failure, error reason is shown in a red callout below the button. Timer cleared on success/failure via `clearInterval`.
