...
- **User-edited Schema Blocks (v1.5.216.29 — Pro+/Agency)** — 5 manual override blocks (Product / Event / LocalBusiness / VacationRental / JobPosting) let users supply authoritative Schema.org values that BYPASS auto-detection for the same `@type`. Override (not merge) — single source of truth per @type per post. See `structured-data.md §4.X` for the per-type required-field spec. Storage: `_seobetter_schema_blocks` post meta. **v1.5.216.62.24 added front-end styled card rendering** — each enabled block now produces a visible HTML card prepended to the post body via `the_content` filter, in addition to the JSON-LD that's already injected into @graph. Card design per type documented in `structured-data.md §4.X`.
