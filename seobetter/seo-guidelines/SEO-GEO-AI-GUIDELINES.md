...
| Listicle | Article + ItemList (one ListItem per numbered pick; **NOT a top-level Product** — that was a v62.71 schema fix) | Intro → Numbered Items (count specified in template, e.g. "10 Numbered Items" → 10 product H2s) → Conclusion | **2500-4000 (raised from 1000-3000 in v1.5.216.62.71)** so the queue's expanded N-product slots each get the §10.1-spec'd 100-200w of content per item. At 2500w with a "10 Numbered Items" template: 15 total slots × 167w avg fits the spec mid-range. |
