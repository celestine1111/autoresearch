...
**Image alt text language coverage + author/featured filter (v1.5.213.2):** v1.5.213's `ImageObject` populate fix exposed two upstream issues. (a) `Stock_Image_Inserter::generate_alt_text()` had 5 hardcoded English templates ("Visual guide explaining %s benefits and considerations" etc.) with no language awareness — Japanese articles ended up with mixed-language alt like `Why このテーマ matter in japan - Best Slow Cooker Recipes for Winter 2026 visual guide` (the keyword-density-translated `このテーマ` slotted into the English template). Fix: non-English path skips templates entirely and uses the section heading directly — headings are guaranteed in target language by the v1.5.212.x heading guard, so this produces clean native alt with zero extra cost. (b) `detect_image_schemas` was emitting standalone `ImageObject` nodes for the author bio photo and featured image. Author photos belong inside `Person.image`, featured image is already in `Article/Recipe.image` — duplicating them as top-level @graph nodes is noise. Fix: skip-list by author profile URL match, featured-image URL match, and class-hint match (`avatar`, `wp-post-image`, `gravatar`, `author-bio`, `seobetter-author`, `icon`, `emoji`, `logo`).

**Localized "this topic" pronoun map (v1.5.213.1):** The keyword-density enforcer at `enforce_geo_requirements()` (v1.5.159) replaces excess focus-keyword mentions with `"this topic"` to keep keyword-density under the 2.5% Princeton-derived ceiling. Pre-fix that string was hardcoded English, so a Japanese article whose body still contained a leaked English keyword (the AI sometimes falls back) had English `"this topic"` injected into Japanese prose (artefact: `「this topic」を探しているなら...`). v1.5.213.1 adds a 33-language BCP-47 map of native pronoun phrases (ja → このテーマ, ko → 이 주제, ru → эта тема, ar → هذا الموضوع, etc.) and skips the `the/a/an + keyword → "it"` branch for non-English (English determiners shouldn't appear in non-English text and "it" would itself be a leak). Fail-open: unknown languages keep "this topic". The map covers all languages in `Localized_Strings::get_language_name()` plus key minority scripts.
...
| `opinion` (v1.5.192, structural-completeness fix v1.5.216.62.66) | **Hybrid** — keeps §3.1's Key Takeaways + FAQ + References, but inserts the Hook & Thesis / Arg 1 / Arg 2 / Arg 3 / The Objection / What This Means / Conclusion + CTA structure. Compliant with both §3.1 AND opinion-genre best practices. **v62.66 — structural completeness now beats word target:** `assemble_markdown()` force-prepends the canonical heading from `$job['headings'][N]` when the AI's section body lacks a `## ` marker (handles the AI rephrasing "Argument 1" → topical heading then dropping the H2 entirely), and `truncate_to_target()` never deletes whole sections — it only trims paragraphs and accepts word-count overshoot rather than dropping a §3.1A section. Opinion may render at 1500-2000w on a 1100w request to fit all 10 sections. | NYT/WaPo/OpEd Project style guides, Purdue OWL, Poynter/Nieman nut-graf research, Princeton GEO (arXiv 2311.09735). See `article_design.md` §11 + BUILD_LOG v1.5.192 / v1.5.216.62.55-66 |
...
| Opinion / Op-Ed | OpinionNewsArticle | **v1.5.192 research-backed structure:** Key Takeaways → Hook+Thesis (nut graf by para 3) → Arg1 (strongest) → Arg2 → Arg3 (REQUIRED, no longer conditional) → The Objection (steelman then refute) → What This Means → FAQ → Conclusion+CTA → References. First-person encouraged, avoid "I think/I feel" hedges. Steelman the opposing view before refuting. Qualified claims beat absolutism. Sources: NYT Opinion/WaPo/OpEd Project guides; Purdue OWL; Poynter/Nieman nut-graf; Princeton GEO (arXiv 2311.09735); Ahrefs AI SEO stats; Claude/Perplexity citation studies Q3 2025; Google E-E-A-T 2025. | **1500-2200 (sweet spot 1500-1800) — raised from 800-1400 in v1.5.216.62.67** so each of the 10 §3.1A sections gets ~150w avg, enough room for the per-section structure (claim + 2-3 evidence + example + transition = 150-220w per Argument). 1100w floor produced cramped Args averaging 110w. |
| How-To Guide | HowTo + Article | Why → Prerequisites → Steps → Troubleshooting → Conclusion | 800-2500 |
| Listicle | Article + ItemList (one ListItem per numbered pick; **NOT a top-level Product** — that was a v62.71 schema fix; v62.108 — ItemList only includes H2s matching `^\d+[.)\s]+\w` numbered pattern, excludes article title/intro/criteria H2s) | Intro → Numbered Items (count specified in template, e.g. "10 Numbered Items" → 10 product H2s) → Conclusion | **2500-4000 (raised from 1000-3000 in v1.5.216.62.71)** so the queue's expanded N-product slots each get the §10.1-spec'd 100-200w of content per item. At 2500w with a "10 Numbered Items" template: 15 total slots × 167w avg fits the spec mid-range. |
...
| **FAQ Page — v1.5.216.62.114b prompt enforcement** | FAQPage | **MANDATORY MIN 10 Q&A pairs (target 12-15)** — anything below 10 is non-compliant. Each H3 ends in "?" (no bundled questions). Each answer 50-100 words (1-2 short paragraphs). Direct answer in FIRST sentence (snippet-friendly). Questions phrased exactly as users would type in search. Cover top 12-15 search queries (mix beginner / intermediate / cost / maintenance / safety / comparison / locale-specific). Topic Intro short (60-100w) — just frames the topic. NO Key Takeaways box (Q&A pairs ARE the takeaways). NO inline body bolds (formatter strips them). | 500-2000 |
| Recipe | Recipe + HowTo (v62.98+99 — recipeIngredient strips Nutrition Facts pollution including plural macros; recipeYield extracted from "Servings Per Recipe: N"; v62.100 — body-keyword cuisine fallback + pasty/scone/muffin category match; v62.104 — prompt now requires AI to emit explicit "Prep Time: X minutes" / "Cook Time: Y minutes" / "Total Time: Z minutes" / "Servings: N" markers in EACH recipe section so Schema_Generator can extract those fields; recipe-specific UK/AU/NZ/IE recipe-site preference) | Story → Tips → Ingredients → Instructions → Notes | 500-1500 |

**v62.106 — Global UK English locale (ALL 21 content types):** When `country` is in `[GB, AU, NZ, IE]` AND language is English, the system prompt at `Async_Generator::get_system_prompt()` appends a LOCALE block instructing the AI to use UK English spelling (flavour / colour / organise / recognise / centre / metre / programme / favourite / behaviour) — NEVER American spellings — plus metric units (grams / ml / celsius) and country currency (£ / AU$ / NZ$ / €). Applies universally to blog_post, listicle, how_to, buying_guide, comparison, news_article, opinion, review, etc. — not just recipes. No-op for US / other countries (byte-identical prompt for them) and for non-English language (Localized_Strings handles those). Test: `tests/test-system-prompt-uk-locale.php`.
...
### 10.4 Schema Notes
- **HowTo** rich results DEPRECATED by Google (Sept 2023) — mapped to Article instead. **Metabox tile dropped in v1.5.216.62.24** — Google does not deliver this rich result anymore so listing it as Available was misleading.
- **FAQPage** rich results restricted to government/health sites — still emitted for AI/semantic value. Auto-emit detects `## Frequently Asked Questions` heading + 3+ Q&A pairs in article body.
...
- **LocalBusiness** — **v1.5.216.62.24 retired the auto-emit-from-article-content path.** Article-style sites do not qualify for the Map Pack rich result (Google reserves it for the business's own page or authoritative aggregators). Now exposed only as a Pro+ manual Schema Block for the legitimate single-business-page case where a customer publishes about *their own* business. Metabox "Local Business / Map Pack" tile dropped.
- **Paywall (`isAccessibleForFree=false`)** — **v1.5.216.62.24 dropped the metabox tile** since the indicator essentially never appears in real Google SERPs for non-publisher sites.
- **DiscussionForumPosting** — **v1.5.216.62.24 dropped the metabox tile** since this `@type` applies to forum software (Reddit, Discourse, vBulletin), not articles.
- **VideoObject (v1.5.216.62.24 — 21-platform detection)** — auto-detected from embeds across YouTube, Vimeo, Rumble, Bilibili, Youku, iQiyi, Niconico, Naver TV, Kakao TV, Dailymotion, Vidio, Aparat, RuTube, VK Video, Coub, TikTok, Twitch, Facebook Watch, Instagram Reels, Wistia, Mux, Brightcove. First-match-wins single VideoObject per article. Per-platform `embed_template` / `content_template` / `thumbnail_template` configs in `Schema_Generator::get_video_platform_configs()`.
- **Speakable** enables Google Assistant to read article aloud — emitted for blog_post, news_article, opinion, pillar_guide, how_to, faq_page, interview, recipe, personal_essay, press_release.
- **User-edited Schema Blocks (v1.5.216.29 — Pro+/Agency)** — 5 manual override blocks (Product / Event / LocalBusiness / VacationRental / JobPosting) let users supply authoritative Schema.org values that BYPASS auto-detection for the same `@type`. Override (not merge) — single source of truth per @type per post. See `structured-data.md §4.X` for the per-type required-field spec. **v1.5.216.62.24** added front-end styled card rendering — each enabled block produces a visible HTML card alongside the JSON-LD that flows into @graph. **v1.5.216.62.28** migrated the entire system from a metabox panel into native Gutenberg blocks under the "SEOBetter" inserter category (`seobetter/product`, `seobetter/event`, `seobetter/local-business`, `seobetter/vacation-rental`, `seobetter/job-posting`). Each block uses InspectorControls for form fields + `wp.serverSideRender` for live editor preview that pixel-matches the published-post output. Multiple instances of the same block type per post are now supported. Inline placement (cards appear at the user's chosen block position rather than prepended to the whole post). Pro+ registration gate: blocks are not registered at all on Free / Pro tier so they don't appear in the inserter. Dual-source JSON-LD emission: `Schema_Blocks_Manager::build_all_jsonld()` reads BOTH Gutenberg blocks in `post_content` (preferred, multiple per type) AND legacy `_seobetter_schema_blocks` post meta (fallback for posts saved pre-v62.28). Per-type dedup so a single @type appears at most once in @graph.
- **Rich Results metabox 2-state model (v1.5.216.62.24)** — replaced the prior 3-state (Active / Available / Not applicable) with 2-state (Active / Not in this article + per-tile action_hint). The amber Available tier was ambiguous; now every Not-Active tile shows an explicit action that activates the rich result OR notes that it doesn't apply to the current content type.
- **Rich Results metabox tile cleanup (v1.5.216.62.25)** — final honest tile count is 13 (was 24 in v62.24, 28 pre-v62.24). v62.25 retired 11 tiles for rich-result lanes that Google does not realistically deliver to article-style sites: recipe_carousel / recipe_gallery / product_carousel / event_carousel / video_carousel / course_carousel / movie_carousel / software_app / dataset / qa_page / profile_page. Schemas for these types are still EMITTED by Schema_Generator (still useful for LLM retrieval); only the metabox tile (which advertises a Google rich-result lane) is removed. Final 13: standard_article, article_with_image, recipe_card, product_card, review_snippet, faq, event_card, video, top_stories, vacation_rental, job_posting, breadcrumbs, speakable.
- **LLM Citation Readiness score refactor (v1.5.216.62.26)** — pre-fix the 8-check score included three speculative per-LLM "bonus" claims (FAQ Perplexity bonus, HowTo ChatGPT bonus, Organization Gemini bonus) with no public research backing them; one (HowTo) flagged a Google-deprecated rich result. Replaced with 9 verifiable signals: title length, description length, featured image dimensions, favicon, site name, **Author+Organization schema (E-E-A-T)**, **inLanguage tag** (Princeton GEO §3 + regional retrieval), **citation[] array** (Princeton GEO §1 — primary source: arxiv 2311.09735, +30% lift), **dateModified ≤90 days** (recency signal universal to all retrievers). Each check is independently auditable; no hardcoded marketing claims surface in the metabox.
- **Schema Impact Estimate stats removed (v1.5.216.62.26)** — the metabox previously displayed unverifiable claims like "+157% organic traffic with video rich results (BrightEdge)", "+87% CTR with FAQ schema (Ahrefs)", "+30-40% AI citation rate (Princeton GEO study)", "Pages with schema rank avg 4 positions higher (Milestone Research, 2023)", "Rich results get 58% of all page 1 clicks (FirstPageSage, 2024)". Some sources existed but the figures were synthesized restatements; others were secondary-blog citations we couldn't trace to primary research. Block removed entirely. Validation block (errors / warnings / passed checks) remains as the trustworthy signal users rely on.
- **Google Discover preview cleanup (v1.5.216.62.26)** — removed the 👍 / 🔖 / ⋯ action-icon row from the Discover preview mock card. Those icons appear in the Google APP Discover feed surface but never on Search SERPs; their presence in the preview was conflating two different rendering surfaces and implying users would see them next to their result on google.com. Card now shows only what's actually in the Discover feed (favicon + site name + title + description).
