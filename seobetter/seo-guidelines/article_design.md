...
**Schema-side ingredient cleanliness (v62.98):** When the recipe section contains a Nutrition Facts `<ul>` block (AI sometimes emits `(Nutrition for one serving)` / `Servings Per Recipe: 6` / `Calories: 558` / `Total Carbohydrate: 51g (19%)` etc), `Schema_Generator::build_recipe()` extended skip filter (line ~1294-1320) strips these from `recipeIngredient[]` so only real ingredients ship to Google. The `Servings Per Recipe: N` value is also captured into `recipeYield`. Visual: nutrition lines may still render in the body if the AI placed them there; schema is the focus of this fix. Test: `tests/test-recipe-ingredient-extraction.php`.
