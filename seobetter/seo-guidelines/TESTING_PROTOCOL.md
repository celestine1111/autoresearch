...
3. If fixing a bug:
   ...
   e. Ship + push + zip + deploy
4. If testing an article:
   a. Trigger Bulk Generate via browse-cli
   b. Poll for new post
   c. Run `full_audit.py` — capture output verbatim
   d. Run external Schema.org validator (NEW — section 3A)
   e. Run browse-cli visible-text scan (NEW — section 3B)
   f. Print FULL audit output to user (NEW — section 3C)
   g. Wait for user approval before marking verified
