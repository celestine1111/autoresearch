...
### Phase 2 testing — SEO-plugin compat, readability uplift, integrations

This block captures the next phase of testing, surfaced during the T3 21-content-type sweep (Day 3 of T3 — 2026-05-08). Each item below is a TEST that needs to PASS, not a feature to design from scratch — most code already exists, but never verified against external plugins / readability targets.

| Test ID | Scope | What to verify | Effort | Origin |
|---|---|---|---|---|
| **PT2.1** | **SEO plugin compatibility — Yoast** | When Yoast SEO is active, SEOBetter's specialty schemas (FAQPage, Recipe, Review, ItemList, HowTo, DefinedTerm, QAPage) STILL ship to the rendered page. **Bug surfaced today:** [seobetter.php:371](../seobetter.php#L371) `output_schema_markup()` early-returns on `defined('WPSEO_VERSION')`, so SEOBetter's schema is silently dropped when Yoast is active — Yoast's default @graph (Article + WebPage + ImageObject + BreadcrumbList + WebSite + Organization + Person) is what reaches the page. **Fix:** register `wpseo_schema_graph` filter to MERGE SEOBetter's specialty types into Yoast's @graph rather than bailing. Verify on Recipe + FAQPage + Review + ItemList + HowTo posts. | 1-2 days | T3 #10 faq_page audit (post 818) — found FAQPage missing despite 10 H3 questions all ending in `?` because Yoast was active |
| **PT2.2** | **SEO plugin compatibility — RankMath** | Same as PT2.1 but for RankMath. Use `rank_math/json_ld` filter to inject SEOBetter specialty schemas into RankMath's @graph. | 1 day | Same root cause as PT2.1 — `seobetter.php:371` early-returns on `class_exists('RankMath')` |
| **PT2.3** | **SEO plugin compatibility — AIOSEO** | Same as PT2.1 but for All-in-One SEO. Use `aioseo_schema_output` filter (or equivalent — verify exact filter name) to inject SEOBetter specialty schemas into AIOSEO's @graph. | 1 day | Same root cause — `seobetter.php:371` early-returns on `defined('AIOSEO_VERSION')` |
| **PT2.4** | **SEO plugin compatibility — SEOPress** | Same as PT2.1 but for SEOPress. Verify SEOPress filter hook (likely `seopress_schemas_single_data`) injects SEOBetter specialty schemas. | 0.5 day | SEOPress meta sync already wired (Phase 1 #1); schema injection is the gap |
| **PT2.5** | **No-SEO-plugin baseline** | Verify SEOBetter's wp_head schema emission ([seobetter.php:104](../seobetter.php#L104)) runs cleanly when NO SEO plugin is active. This is the path that fires today on dev — should not regress when PT2.1-PT2.4 land. | 0.25 day | Regression coverage — keep wp_head emission as fallback for no-SEO-plugin sites |
| **PT2.6** | **SEO plugin meta sync — Yoast** | Verify Phase 1 #1 (canonical URL sync to all 4 SEO plugins) — title, meta description, focus keyword, OG image, Twitter image all populate Yoast meta keys correctly when an article is generated. Re-test on each Yoast major version. | 0.5 day | Already wired ([seobetter.php:3296-3311](../seobetter.php#L3296)); needs cross-version verification |
| **PT2.7** | **SEO plugin meta sync — RankMath / AIOSEO / SEOPress** | Same as PT2.6 for the other 3 plugins. Each has its own meta key naming. | 1 day | Same as PT2.6 |
| **PT2.8** | **Readability gate — Flesch ≥ 60** | Generate one article per content type and verify Flesch Reading Ease ≥ 60 (target 60-70+, higher is fine). **Today's baseline (Day 3 sweep on signed-off posts):** all 8 verified posts FAIL — Flesch range 46.8 to 52.8, mean ~50. Yoast also reports ~50 on most articles. Drives a prose-prompt revision: shorter sentences (12-15 words avg, was 15-18+), lower-syllable word selection, more 1-2 syllable verbs. | 2-3 days | Today's audit additions surfaced this — pre-T3, Flesch was N/A |
| **PT2.9** | **Readability gate — Island Test (0 pronoun starts)** | Verify each H2/H3 section's first paragraph does NOT start with a pronoun (He/She/It/They/We/You/This/That/These/Those/There/Such). **Today's baseline:** 7 of 8 posts have at least 1 violation; post 794 + 802 + 818 each have 2. Pronoun starts break AI extractability — answer-engine citation needs each section self-contained without referring-back context. Drives a prose-prompt rule: "Section openers must repeat the noun, never refer back with a pronoun." | 1-2 days (prompt rule + retest) | Same Day-3 audit additions |
| **PT2.10** | **Readability gate — Section openings 40-60 words** | Verify ≥75% of H2/H3 section first-paragraphs are 40-60 words. **Today's baseline:** 1-8 sections out of 7-14 hit the band per post (range 14-57% compliance, target 75%). Drives a prose-prompt rule: "Each section's opening paragraph must be 40-60 words — long enough to be a self-contained answer, short enough to fit in an AI extraction window." | 1-2 days | Same Day-3 audit additions |
| **PT2.11** | **eBay API integration** | Stand up eBay finder API for product-rich listicle / buying_guide / review enrichment. Currently price/availability data is AI-hallucinated for non-Amazon merchants — eBay API would fill the gap for second-hand + niche products. Vercel proxy endpoint + cache layer. Mirror the Amazon PA-API pattern but eBay-specific. | 3-5 days | User-mentioned 2026-05-08 — pairs with Verified Prices pipeline (Phase 2 already-listed) |
| **PT2.12** | **Verified prices — multi-source cross-check** | Already in Phase 2 backlog as `verified-prices.md` Phase 2 sister project. Surface here for visibility: Amazon PA-API + eBay API + Walmart + retailer JSON-LD scrape, all cached, surfaced as `data-verified-price` attribute on price callouts. Reject any AI-emitted price not backed by ≥1 source. | 5-7 days | Phase 2 already specs; add to test list once endpoints land |
| **PT2.13** | **Verified citations — multi-source cross-check** | Already in Phase 2 backlog as `verified-citations.md`. Same pattern as PT2.12 but for quote attribution + statistic claims. Reject any AI-emitted quote/stat not backed by ≥1 verifiable source. | 5-7 days | Already documented; add to test list once endpoints land |
| **PT2.14** | **Multilingual schema + readability** | Verify schema validity + Flesch-equivalent (Lex Alemana / Spache / Fernández-Huerta scoring per language) on generated articles in fr/de/es/pt/ja/ko/zh. Each language has its own readability formula — wire up per-locale scoring in `tests/full_audit.py`. | 2-3 days | Phase 1 T10 multilingual smoke didn't include readability — adds it post-Phase-1 |
| **PT2.15** | **Country sources expansion verification** | Verify the country-localised `tests/last30days/` Python scraper still returns sources for all 89 countries in the Pro+ allowlist. Periodic regression (sources rot — sites disappear, structure changes). Re-run weekly in CI; fail loud when source count drops below 100/country. | 2 days + ongoing CI | Phase 2 sister sprint, already documented in pro-features-ideas backlog history |
| **PT2.16** | **Schema_Generator — `_seobetter_schema` meta vs inline `<!-- wp:html -->`** | Decide once whether the wp:html embed path ([seobetter.php:516](../seobetter.php#L516)) should still fire, or whether `_seobetter_schema` post meta + wp_head emission ([seobetter.php:386](../seobetter.php#L386)) is the single source of truth. Today's audit showed both 816 (recipe) + 818 (faq_page) had 0 inline JSON-LD blocks in `post_content` — so the wp:html path isn't running consistently. Either fix it OR remove the dead code. | 0.5 day | Surfaced today — code-path inconsistency |

**Sequencing recommendation:**

1. **PT2.5** first (regression coverage — confirm baseline still works)
2. **PT2.1 (Yoast)** next — biggest user impact, blocks faq_page + recipe sign-offs from being valid for Yoast users
3. **PT2.2-PT2.4** (RankMath / AIOSEO / SEOPress) in parallel as a single sprint
4. **PT2.6-PT2.7** (meta sync verification) alongside PT2.1-PT2.4 since they touch the same plugins
5. **PT2.8-PT2.10** (readability gate) — this is one prose-prompt revision driving all 3, ship together
6. **PT2.11** (eBay API) — independent, can be parallel-tracked
7. **PT2.12-PT2.13** (verified data pipelines) — once eBay lands, the cross-check pattern extends to it
8. **PT2.14** (multilingual readability) — after PT2.8 validates the Flesch path
9. **PT2.15** (country sources CI) — ongoing
10. **PT2.16** (schema code-path consolidation) — clean-up at any time

**Phase 2 sign-off gate:** ALL 16 tests above pass before Freemius launch (Phase 3 prep). The Yoast / RankMath / AIOSEO compatibility items (PT2.1-PT2.4) are the highest-priority blockers because the plugin's core value prop ("specialty rich-result schemas") silently fails on the majority of WordPress sites today, which run one of those 3 SEO plugins.
