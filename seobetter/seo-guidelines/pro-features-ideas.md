...
| **🚨 BLOCKER: Centralized GSC OAuth proxy** — Phase 1 ships GSC connect with each install registering its own Google OAuth client → every user sees Google's "Google hasn't verified this app" warning when authorizing. **Cannot ship to paying customers in this state.** Build path: (1) register `seobetter.com` OAuth app with Google, (2) submit for Google verification (~1-2 weeks for non-sensitive read-only Search Console scopes — requires privacy policy + ToS at seobetter.com + domain verification), (3) deploy Cloudflare Worker / Vercel function proxy at `seobetter.com/oauth/gsc/{authorize,callback}`, (4) plugin update — point at proxy instead of user-supplied `SEOBETTER_GSC_CLIENT_ID`/`SECRET` constants. **Schedule 2-3 weeks before public launch** (Google verification timeline + buffer). Infra cost: ~$0/mo at low scale (Cloudflare Workers free tier). After this ships, end users get clean OAuth flow with verified-app green check, no scary warning. | 1-2 weeks |
