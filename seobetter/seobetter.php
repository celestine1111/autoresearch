...
define( 'SEOBETTER_VERSION', '1.5.182' );
...
final class SEOBetter {
    ...
    private function init_hooks(): void {
        ...
        // v1.5.182 — Schema now served from post meta via wp_head only.
    ...
    public function register_admin_menu(): void {
        ...
        // v1.5.182 — Removed 4 empty menu items (Content Brief, Citation Tracker, Link Suggestions, Cannibalization).
    ...
    public function output_schema_markup(): void {
        ...
        $post_id = get_the_ID();
        // v1.5.182 — Always output schema from post meta via wp_head.
    ...
    public function analyze_on_save( int $post_id, \WP_Post $post ): void {
        ...
        $settings = get_option( 'seobetter_settings', [] );
        // v1.5.182 — Schema generation always runs regardless of auto_analyze.
        ...
        try {
            ...
            // v1.5.182 — Schema ALWAYS regenerated on save (not gated by auto_analyze
    ...
    /**
     * v1.5.182 — Regenerate schema with correct permalink when post is published.
    ...
    public function rest_analyze( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        if ( $schema ) {
            ...
            // v1.5.182 — Pass full schema for Rich Results Preview
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.182 — Convert bracketed text references to real links.
        ...
        if ( ! empty( $schema_array ) ) {
            ...
            // v1.5.182 — Schema stored in post meta ONLY (not inline in post_content).
    ...
    public static function cleanup_ai_markdown( string $md ): string {
        ...
        // 5. v1.5.182 — Strip academic/Crossref junk text from AI output.
    ...
    public function rest_optimize_all( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        $country = sanitize_text_field( $request->get_param( 'country' ) ?? '' );
        // v1.5.182 — Content-type-aware optimization mode
        ...
        // v1.5.182 — Skip validate_outbound_links after optimization.
        ...
        // v1.5.182 — Convert bracketed/parenthetical text references to links.
     ...
     *
    /**
     * v1.5.182 — Convert bracketed text references into real markdown links.
     *
    ...
    public static function linkify_bracketed_references( string $markdown, array $pool ): string {
        ...
        // v1.5.182 — Rewritten to handle nested parens in source titles like
        ...
        // v1.5.182 — Second pass: handle deeply nested parens like
        // "(Python API Tutorial (Beginner's Guide) | Moesif Blog)".
        ...
        foreach ( $lines as $li => $line ) {
            ...
            // v1.5.182b — REMOVED the old "skip if line has ](http and balanced parens" check.
    ...
    private function validate_outbound_links( string $markdown, array $citation_pool = [] ): string {
        ...
        $filter_link = function ( $url, $text ) use ( $whitelist, $site_host, $citation_pool ) {
            ...
            //
            // v1.5.182 — Block DOI/academic URLs (often 404, not reader-friendly)
            if ( preg_match( '/^(doi\.org|dx\.doi\.org)$/i', $host ) ) {
                return [ 'keep' => false, 'text' => $text ];
            }
            // v1.5.182 — Block raw data file URLs and API query endpoints
    ...
    private function append_references_section( string $markdown, array $citation_pool ): string {
        ...
        foreach ( $cited_entries as $entry ) {
            ...
            // v1.5.182 — Sanitize title for markdown link safety.
