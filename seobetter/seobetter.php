...
define( 'SEOBETTER_VERSION', '1.5.160' );
...
final class SEOBetter {
    ...
    private function init_hooks(): void {
        ...
        // v1.5.160 — Schema now served from post meta via wp_head only.
    ...
    public function output_schema_markup(): void {
        ...
        $post_id = get_the_ID();
        // v1.5.160 — Always output schema from post meta via wp_head.
    ...
    public function analyze_on_save( int $post_id, \WP_Post $post ): void {
        ...
        $settings = get_option( 'seobetter_settings', [] );
        // v1.5.160 — Schema generation always runs regardless of auto_analyze.
        ...
        try {
            ...
            // v1.5.160 — Schema ALWAYS regenerated on save (not gated by auto_analyze
    ...
    /**
     * v1.5.160 — Regenerate schema with correct permalink when post is published.
    ...
    public function rest_analyze( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        if ( $schema ) {
            ...
            // v1.5.160 — Pass full schema for Rich Results Preview
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.160 — Convert bracketed text references to real links.
        ...
        if ( ! empty( $schema_array ) ) {
            ...
            // v1.5.160 — Schema stored in post meta ONLY (not inline in post_content).
    ...
    public static function cleanup_ai_markdown( string $md ): string {
        ...
        // 5. v1.5.160 — Strip academic/Crossref junk text from AI output.
    ...
    public function rest_optimize_all( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        $country = sanitize_text_field( $request->get_param( 'country' ) ?? '' );
        // v1.5.160 — Content-type-aware optimization mode
        ...
        // v1.5.160 — Skip validate_outbound_links after optimization.
        ...
        // v1.5.160 — Convert bracketed/parenthetical text references to links.
     ...
     *
    /**
     * v1.5.160 — Convert bracketed text references into real markdown links.
     *
    ...
    private function validate_outbound_links( string $markdown, array $citation_pool = [] ): string {
        ...
        $filter_link = function ( $url, $text ) use ( $whitelist, $site_host, $citation_pool ) {
            ...
            //
            // v1.5.160 — Block DOI/academic URLs (often 404, not reader-friendly)
            if ( preg_match( '/^(doi\.org|dx\.doi\.org)$/i', $host ) ) {
                return [ 'keep' => false, 'text' => $text ];
            }
            // v1.5.160 — Block raw data file URLs and API query endpoints
    ...
    private function append_references_section( string $markdown, array $citation_pool ): string {
        ...
        foreach ( $cited_entries as $entry ) {
            ...
            // v1.5.160 — Sanitize title for markdown link safety.
