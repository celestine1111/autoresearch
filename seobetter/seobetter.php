...
define( 'SEOBETTER_VERSION', '1.5.216.62.114' );
...
final class SEOBetter {
    ...
    public function register_admin_menu(): void {
        ...
        add_submenu_page( 'seobetter', __( 'Internal Links', 'seobetter' ), __( 'Internal Links', 'seobetter' ), 'edit_posts', 'seobetter-internal-links', [ $this, 'render_link_suggestions' ] );
    ...
    public function register_rest_routes(): void {
        ...
        // v1.5.216.62 — GSC property picker endpoints
        ...
        // v1.5.216.62 — Per-post Freshness diagnostic ("Why?" drawer + metabox tab + Gutenberg sidebar).
        ...
        // v1.5.216.62 — DEBUG-only test data seeders. Gated by WP_DEBUG at the
    ...
    public function rest_gsc_oauth_callback( \WP_REST_Request $request ): \WP_REST_Response {
        // v1.5.216.62 — Two arrival modes:
        //   Proxy mode (default): user arrives via Cloud-OAuth proxy with `pickup` + `state`
        //   BYO mode (legacy):    user arrives directly from Google with `code` + `state`
        // GSC_Manager::handle_oauth_callback handles both — it dispatches based on use_proxy().
    ...
    /**
     * v1.5.216.62 — list every GSC property the authorized account can access.
    ...
    /**
     * v1.5.216.62 — set which GSC property the plugin syncs.
    ...
    /**
     * v1.5.216.62 — DEBUG-only: insert mock GSC snapshots so Freshness/Decay/
    ...
    /**
     * v1.5.216.62 — return per-post Freshness diagnostic for the "Why?" drawer
    ...
    /**
     * v1.5.216.62.114 — Sanitize headline before it becomes post_title.
     *
     ...
     */
    public static function brand_caps( string $title ): string {
        $tokens = [
    ...
    /**
     * v1.5.216.62.114 — Universal pre-emptive cleaner. Pool comparison alone
     ...
     */
    public static function clean_title_universal( string $title ): string {
        $title = trim( $title );
    ...
    public static function sanitize_headline( string $title, string $fallback_keyword, array $pool = [] ): string {
        $title = trim( $title );
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        $raw_title = (string) ( $request->get_param( 'title' ) ?? 'New Article' );
        $markdown = $request->get_param( 'markdown' ) ?? '';
        $content  = $request->get_param( 'content' ) ?? '';
        $accent   = sanitize_text_field( $request->get_param( 'accent_color' ) ?? '#764ba2' );

        // v1.5.216.62.114 — sanitize headline AFTER citation_pool is parsed below
        ...
        // v1.5.216.62.114 — Now that the pool is assembled, sanitize the
            ...
            if ( ! empty( $combined_pool )
                ...
                if ( ! empty( $cited_entries ) ) {
                    ...
                    foreach ( $cited_entries as $entry ) {
                        ...
                        if ( $url === '' ) continue;
                        // v1.5.216.62.114 — truthy fallback chain. Pre-fix used
                        ...
                        foreach ( [ 'title', 'source_name' ] as $key ) {
                            ...
                            if ( $candidate !== '' ) { $title_raw = $candidate; break; }
                        }
                        if ( $title_raw === '' ) {
                            $host = wp_parse_url( $entry['url'] ?? '', PHP_URL_HOST );
                            $title_raw = is_string( $host ) && $host !== '' ? $host : ( $entry['url'] ?? 'Source' );
                        }
    ...
    public function rest_improve_content( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        return new \WP_REST_Response( [
            ...
            'geo_score'  => $score['geo_score'],
            'grade'      => $score['grade'],
            // v1.5.216.62 — language-aware word count
     ...
     */
    public function sync_seo_plugin_meta( int $post_id, string $meta_title, string $meta_desc, string $keyword, string $content_type = '' ): void {
    ...
    /**
     * v1.5.216.62.63 — Source-quality URL test (centralized helper).
     *
     * Returns true when the URL fails the v62.62 source-quality filters
     * (Reddit non-allowlist subs, LinkedIn non-pulse, Medium personal-story,
     * Hacker News, Quora). Used by:
     *   - validate_outbound_links()::filter_link()       — strips AI-written links
     *   - linkify_bracketed_references()                 — drops pool entries before linkifying plain-text (Source) mentions
     *   - append_references_section()                    — excludes from auto-built References list
     *   - Citation_Pool::append_references_section()     — same exclusion in the shared path
     *
     * Pre-fix v62.62 only ran the filters in validate_outbound_links's
     * inner closure. linkify and references_section pulled URLs straight
     * from the citation pool, bypassing the filter — Bluesky / HN /
     * non-allowlisted Reddit / LinkedIn-personal URLs got re-injected
     * after validate stripped the AI's own attempts. User-reported on the
     * v62.62 retest: 6 bsky.app links + 2 ycombinator + 3 r/trustandsafetypros
     * (not in allowlist) all came through linkify.
     *
     * Centralizing here makes adding new low-trust hosts a one-line
     * change. All consumers see the same filter.
     *
     * Returns:
     *   true  → URL is low-quality, should be stripped from the article
     *   false → URL passes (or filter doesn't apply to its host)
     */
    public static function is_low_quality_source( string $url ): bool {
        $host = wp_parse_url( $url, PHP_URL_HOST ) ?? '';
        $path = wp_parse_url( $url, PHP_URL_PATH ) ?? '';
        if ( $host === '' ) return false;

        // Reddit — allowlisted subreddits only
        if ( preg_match( '#(?:^|\.)reddit\.com$#i', $host ) ) {
            if ( ! preg_match( '#^/r/([^/]+)/comments/#i', $path, $sub_m ) ) {
                return true; // not a comments-thread URL
            }
            $subreddit = strtolower( $sub_m[1] );
            static $allowed_subreddits = [
            ...
            return ! in_array( $subreddit, $allowed_subreddits, true );
        }

        // LinkedIn — allow only /pulse/named-author-slug
        if ( preg_match( '#(?:^|\.)linkedin\.com$#i', $host ) ) {
            return ! preg_match( '#^/pulse/[a-z0-9-]+#i', $path );
        }

        // Medium — block @username + bare title-hash personal stories
        if ( preg_match( '#(?:^|\.)medium\.com$#i', $host ) ) {
            if ( preg_match( '#^/@#', $path ) ) return true;
            if ( preg_match( '#^/[a-z0-9][a-z0-9-]*-[a-f0-9]{8,}/?$#i', $path ) ) return true;
            return false;
        }

        // Hacker News — blocked entirely
        if ( preg_match( '#(?:^|\.)ycombinator\.com$#i', $host ) ) return true;

        // Quora — blocked entirely
        if ( preg_match( '#(?:^|\.)quora\.com$#i', $host ) ) return true;

        // Personal social feeds (post-v62.61 these aren't in the static whitelist
        // anymore but pool-membership could still surface them — block here too)
    ...
    public static function linkify_bracketed_references( string $markdown, array $pool ): string {
        ...
        // v1.5.216.62.63 — Pre-filter the pool through the source-quality
        // helper. Pre-fix linkify pulled directly from the pool, so URLs
        // that validate_outbound_links would have stripped (Bluesky, HN,
        // non-allowlisted Reddit, LinkedIn-personal, Medium-personal)
        // still made it into the body when linkify wrapped a matching
        // (Source) plain-text mention. Now low-quality entries are
        // skipped entirely — the bracket stays as plain text rather than
        // becoming a clickable bad link.
        $pool = array_values( array_filter( $pool, function ( $entry ) {
            $url = $entry['url'] ?? '';
            return $url !== '' && ! self::is_low_quality_source( $url );
        } ) );
        if ( empty( $pool ) ) return $markdown;
        ...
        // v1.5.216.62 — also match full-width parens （ … ） + Japanese sentence
        ...
        // v1.5.216.62.114 — Length cap raised 150 → 250 to catch long parenthetical
        // citations like "(Top 5 Best Automatic Cat Feeders of 2026: Why PalNests
        // Wins)" which exceeded the prior cap and stayed as plain text in body.
        // Found by user audit on T3 #8 post 757 (cat feeders Buying Guide). Test
        // coverage in tests/test-linkify-parens.php.
        $markdown = preg_replace_callback(
            '/([\(（])([^)）]{2,250})([\)）])(?=[.\s,;!?\n。！？、]|$)/mu',
        ...
        // v1.5.216.62 — walks both half-width () and full-width （） so JA
     ...
     */
    public function validate_outbound_links( string $markdown, array $citation_pool = [] ): string {
        // ===== Pass -0.5: Scrub plaintext social-platform handles =====
        // v1.5.216.62.66 — The v62.61 / v62.62 / v62.63 filters strip
        // bsky / mastodon / lemmy URLs, but they only see anchor `<a href>`
        // hrefs and bracketed [text](URL) markdown. Bare handles typed
        // into prose by the AI ("@kcox105.bsky.social writes that...")
        // bypass every filter because there's no URL to inspect. T3 #3
        // Opinion v62.65 retest: bsky handle still appeared in body even
        // though zero bsky URLs survived the link filter.
        //
        // Conservative scrub — only handles whose suffix unambiguously
        // identifies a personal-social-feed platform we already excluded
        // from the citation whitelist. Twitter / X handles are NOT
        // scrubbed because @username is the canonical reference style
        // for X and many credible authors are X-only. Quora @username
        // is also kept because Quora bans / restrictions are URL-only.
        $markdown = preg_replace(
            [
                '#\s*\(?@[\w._-]+\.bsky\.(?:app|social)\)?#i',
                '#\s*\(?@[\w._-]+@mastodon\.[a-z0-9.-]+\)?#i',
                '#\s*\(?@[\w._-]+@lemmy\.[a-z0-9.-]+\)?#i',
            ],
            '',
            (string) $markdown
        ) ?? $markdown;
        // Clean up any double-spaces / orphaned punctuation the scrub left behind
        $markdown = preg_replace( '/\s+([.,;:!?])/', '$1', (string) $markdown ) ?? $markdown;
        $markdown = preg_replace( '/\(\s*\)/', '', (string) $markdown ) ?? $markdown;
        $markdown = preg_replace( '/[ \t]{2,}/', ' ', (string) $markdown ) ?? $markdown;
        ...
        $filter_link = function ( $url, $text ) use ( $whitelist, $site_host, $citation_pool, $sanitize_mashed_url ) {
            ...
            // v1.5.216.62.63 — refactored to call the centralized helper
            // SEOBetter::is_low_quality_source() so linkify_bracketed_references
            // and append_references_section share the same filter logic.
            // The inline regex chain that used to live here is now in the
            // helper method (one source of truth for low-trust URL patterns).
            if ( self::is_low_quality_source( $url ) ) {
                return [ 'keep' => false, 'text' => $text ];
            }
    ...
    public function set_featured_image( int $post_id, string $keyword ): void {
    ...
    public static function schema_block_field_defs( string $type ): array {
        switch ( $type ) {
            ...
            case 'localbusiness':
                // v1.5.216.62.38 — full 96-type business_type taxonomy lives
                // in assets/js/schema-blocks.js DEFS.localbusiness (JS is the
                // source of truth for the dropdown options). PHP only needs
                // the field key here so build_attributes_from_field_defs()
                // registers `business_type` as a string attribute. The list
                // below is a representative sample for any non-Gutenberg
                // entry path (e.g. metabox fallback).
                return [
