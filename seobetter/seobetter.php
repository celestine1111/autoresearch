...
define( 'SEOBETTER_VERSION', '1.5.216.62.91' );
...
final class SEOBetter {
    ...
    /**
     * v1.5.216.62.91 — Sanitize headline before it becomes post_title.
     *
    ...
    /**
     * v1.5.216.62.91 — Universal pre-emptive cleaner. Pool comparison alone
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.216.62.91 — sanitize headline AFTER citation_pool is parsed below
        ...
        // v1.5.216.62.91 — Now that the pool is assembled, sanitize the
            ...
            if ( ! empty( $combined_pool )
                ...
                if ( ! empty( $cited_entries ) ) {
                    ...
                    foreach ( $cited_entries as $entry ) {
                        ...
                        if ( $url === '' ) continue;
                        // v1.5.216.62.91 — truthy fallback chain. Pre-fix used
