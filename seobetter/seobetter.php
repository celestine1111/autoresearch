...
define( 'SEOBETTER_VERSION', '1.5.216.62.104' );
...
final class SEOBetter {
    ...
    /**
     * v1.5.216.62.104 — Sanitize headline before it becomes post_title.
     *
    ...
    /**
     * v1.5.216.62.104 — Universal pre-emptive cleaner. Pool comparison alone
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.216.62.104 — sanitize headline AFTER citation_pool is parsed below
        ...
        // v1.5.216.62.104 — Now that the pool is assembled, sanitize the
            ...
            if ( ! empty( $combined_pool )
                ...
                if ( ! empty( $cited_entries ) ) {
                    ...
                    foreach ( $cited_entries as $entry ) {
                        ...
                        if ( $url === '' ) continue;
                        // v1.5.216.62.104 — truthy fallback chain. Pre-fix used
    ...
    public static function linkify_bracketed_references( string $markdown, array $pool ): string {
        ...
        // v1.5.216.62.104 — Length cap raised 150 → 250 to catch long parenthetical
