...
define( 'SEOBETTER_VERSION', '1.5.65' );
...
final class SEOBetter {
    ...
    public function register_rest_routes(): void {
        ...
        // v1.5.65 — diagnostic endpoint that tests the full Places Sonar
        ...
        // v1.5.65 — diagnostic endpoint for Foursquare + HERE + Google Places.
        ...
        // v1.5.65 — diagnostic endpoint for every always-on research source
    ...
    /**
     * v1.5.65 — Test Sonar connection diagnostic endpoint.
     *
    ...
    public function rest_test_sonar( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.65 — accept keyword / country / domain from the request so the
        ...
        try {
            ...
            return new \WP_REST_Response( [
                ...
                // v1.5.65 — pass the resolved location into the verdict so
     ...
     * v1.5.65 — verdict strings no longer hardcode "Lucignano". The tested
    ...
    /**
     * v1.5.65 — Test Foursquare / HERE / Google Places directly, bypassing
        ...
        try {
            ...
            if ( ! empty( $fsq_key ) ) {
                if ( isset( $per_tier['Foursquare'] ) ) {
                    ...
                    $verdict_lines[] = "❌ Foursquare: key configured but the cloud-api NEVER called the Foursquare tier. This usually means the cloud-api Vercel deployment is outdated. Check Vercel → seobetter-cloud → latest deployment is >= v1.5.65.";
                }
            } else {
            ...
            if ( ! empty( $here_key ) ) {
                if ( isset( $per_tier['HERE'] ) ) {
                    ...
                    $verdict_lines[] = "❌ HERE: key configured but the cloud-api NEVER called the HERE tier. Check Vercel deployment is >= v1.5.65.";
                }
            } else {
    ...
    /**
     * v1.5.65 — Test every research source (cloud-api + local Last30Days)
    ...
    public function rest_generate_start( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.65 — wrap start_job in a try/catch so any thrown exception
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        if ( ! empty( $markdown ) ) {
            ...
            // v1.5.65 — run Places_Link_Injector on the saved hybrid HTML so
    ...
    public function rest_inject_fix( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        switch ( $fix_type ) {
            ...
            case 'statistics':
                ...
                break;
            case 'readability':
                // v1.5.65 — now runs an inject-mode AI rewriter pass that
            ...
            case 'openers':
                ...
                return new \WP_REST_Response( $result );
            // v1.5.65 — three missing flag handlers that were wired in the UI
        ...
        $formatter = new SEOBetter\Content_Formatter();
        // v1.5.65 — REVERTED v1.5.62/63's switch back to 'classic' mode.
    ...
    private function append_references_section( string $markdown, array $citation_pool ): string {
        ...
        // v1.5.65 — when the AI forgot to use [Source](url) format inline
        ...
        foreach ( $cited_entries as $entry ) {
            ...
            // v1.5.65 — removed the " — {$src}" suffix. User feedback:
    ...
    private function get_trusted_domain_whitelist(): array {
        $default = [
            ...
            // v1.5.65 — Perplexity Sonar (Tier 0) scrapes these tourism and
            ...
            // v1.5.65 — common Brave Search result domains that return
    ...
    private function set_featured_image( int $post_id, string $keyword ): void {
        ...
        // v1.5.65 — Branding AI image generation first. Try the user's
