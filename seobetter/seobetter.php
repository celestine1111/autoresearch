...
define( 'SEOBETTER_VERSION', '1.5.57' );
...
final class SEOBetter {
    ...
    public function register_rest_routes(): void {
        ...
        // v1.5.57 — diagnostic endpoint that tests the full Places Sonar
        ...
        // v1.5.57 — diagnostic endpoint for Foursquare + HERE + Google Places.
        ...
        // v1.5.57 — diagnostic endpoint for every always-on research source
    ...
    /**
     * v1.5.57 — Test Sonar connection diagnostic endpoint.
     *
    ...
    public function rest_test_sonar( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.57 — accept keyword / country / domain from the request so the
        ...
        try {
            ...
            return new \WP_REST_Response( [
                ...
                // v1.5.57 — pass the resolved location into the verdict so
     ...
     * v1.5.57 — verdict strings no longer hardcode "Lucignano". The tested
    ...
    /**
     * v1.5.57 — Test Foursquare / HERE / Google Places directly, bypassing
        ...
        try {
            ...
            if ( ! empty( $fsq_key ) ) {
                if ( isset( $per_tier['Foursquare'] ) ) {
                    ...
                    $verdict_lines[] = "❌ Foursquare: key configured but the cloud-api NEVER called the Foursquare tier. This usually means the cloud-api Vercel deployment is outdated. Check Vercel → seobetter-cloud → latest deployment is >= v1.5.57.";
                }
            } else {
            ...
            if ( ! empty( $here_key ) ) {
                if ( isset( $per_tier['HERE'] ) ) {
                    ...
                    $verdict_lines[] = "❌ HERE: key configured but the cloud-api NEVER called the HERE tier. Check Vercel deployment is >= v1.5.57.";
                }
            } else {
    ...
    /**
     * v1.5.57 — Test every research source (cloud-api + local Last30Days)
    ...
    public function rest_generate_start( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        // v1.5.57 — wrap start_job in a try/catch so any thrown exception
    ...
    public function rest_save_draft( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        if ( ! empty( $markdown ) ) {
            ...
            // v1.5.57 — run Places_Link_Injector on the saved hybrid HTML so
    ...
    private function append_references_section( string $markdown, array $citation_pool ): string {
        ...
        foreach ( $cited_entries as $entry ) {
            ...
            // v1.5.57 — removed the " — {$src}" suffix. User feedback:
    ...
    private function get_trusted_domain_whitelist(): array {
        $default = [
            ...
            // v1.5.57 — Perplexity Sonar (Tier 0) scrapes these tourism and
    ...
    private function set_featured_image( int $post_id, string $keyword ): void {
        ...
        // v1.5.57 — Branding AI image generation first. Try the user's
