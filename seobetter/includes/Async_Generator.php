...
class Async_Generator {
    ...
    public static function start_job( array $params ): array {
        ...
        //
        // v1.5.216.62.67 — opinion floor raised again 1100 → 1500. After
        // v62.66 fixed truncate_to_target to never delete whole sections,
        // 1100w / 10 sections = ~110w per section was producing cramped
        // sections that lacked room for the §3.1A guidance's per-section
        // structure (claim sentence + 2-3 evidence sentences + named
        // source + example + transition = 150-220w per Argument). The
        // 1500w floor gives ~150w/section average, enough for substantive
        // arguments without forcing the AI to truncate evidence. Updated
        // §10.1 word range from "800-1400, sweet 900-1100" → "1500-2200,
        // sweet 1500-1800" in same commit. User-requested 2026-05-05.
        ...
        $min_words = [
            ...
            'sponsored' => 800,
            'press_release' => 400, 'glossary_definition' => 500,
        ];
    ...
    public static function process_step( string $job_id ): array {
        ...
        // v1.5.216.9 — Translate the focus keyword for non-English articles
        // BEFORE the prompt-building uses it. Pre-fix: prompts at line ~1022
        // and ~1173 told the AI "use the primary keyword '{$keyword}'
        // 1-2 times" + "headings MUST contain the exact phrase
        // '{$keyword}'", and line 2663's LANGUAGE rule explicitly allowed
        // keeping the keyword in English. Result: French articles dutifully
        // included "Best Ramen Shops In Montreal 2026" verbatim in the
        // French body.
        //
        // Mirrors v1.5.213.3 which already translates keywords for
        // generate_headlines + generate_meta_tags. Now applies to article
        // body generation too. Single OpenRouter call cached per article
        // because it runs once at the top of run_step (which the WP cron
        // processes step-by-step, but the job is the same).
        //
        // Original English keyword preserved on the job in case downstream
        // technical SEO fields (focus keyword post meta) need it.
        $language_code = $options['language'] ?? 'en';
        $base_lang = strtolower( substr( $language_code, 0, 2 ) );
        $original_keyword = $keyword;
        if ( $base_lang !== 'en' && $base_lang !== '' && trim( $keyword ) !== '' ) {
            // Cache the translation on the job so we don't re-translate
            // every time run_step processes a new step for the same job.
            if ( ! empty( $job['translated_keyword'] ) ) {
                $keyword = $job['translated_keyword'];
            } else {
                $batch = \SEOBetter\Cloud_API::translate_strings_batch( [ $keyword ], $language_code );
                if ( is_array( $batch ) && ! empty( $batch[0] ) && $batch[0] !== $keyword ) {
                    $keyword = $batch[0];
                    // Persist on the job so subsequent steps reuse it
                    $job['translated_keyword'] = $keyword;
                }
            }
        }
        ...
        try {
            if ( $step === 'trends' ) {
                ...
                // v1.5.216.62.47 — also thread content_type so the formula
                // table for genre-override types (news_article / opinion /
                // recipe / personal_essay / press_release / live_blog /
                // interview) doesn't seed the headline prompt with the
                // default 5 formulas (Number / How-to / Question / Power /
                // Year). Genre-correct formulas defined in
                // AI_Content_Generator::headline_formulas_for_type().
                $headlines = $generator->generate_headlines(
                    $keyword,
                    wp_strip_all_tags( $article_text ),
                    $options['language'] ?? 'en',
                    $options['content_type'] ?? 'blog_post'
                );
    ...
    private static function get_prose_template( string $content_type, int $recipe_source_count = 3, string $country = '' ): array {
        $templates = [
            ...
            // Pre-fix: Key Takeaways was in the sections list — directly
            // contradicting SEO-GEO-AI-GUIDELINES.md §3.1A which says
            // "news_article ... No standalone Key Takeaways." User-reported
            // on the RBA-rate article: AI dutifully emitted a Key Takeaways
            // H2 above the lede because the template said to. News
            // convention is inverted-pyramid lede first, no editorial
            // summary box. GEO_Analyzer already SKIPS bluf_header for
            // news_article (returns 100, no penalty for missing it), so
            // removing Key Takeaways here costs zero score and fixes
            // genre authenticity. Headline guidance also added: the
            // headline must report an event (subject + active verb + 5
            // Ws), not a how-to / guide pattern — fixes the user-observed
            // "How-to Prepare for the australian rba interest rate
            ...
            'opinion' => ['sections' => 'Key Takeaways, Hook and Thesis, Argument 1, Argument 2, Argument 3, The Objection, What This Means, FAQ, Conclusion and Call to Action, References', 'guidance' => 'Argumentative op-ed with an explicit, provable thesis stated by paragraph 3 at the latest. SECTION PURPOSE: "Hook and Thesis" opens with a 1-3 sentence anecdote/stat/provocation, then an explicit thesis sentence, then a 1-2 sentence preview of your 2-3 arguments. "Argument 1" is your STRONGEST point — claim sentence + 2-3 evidence sentences (named source + number) + 1 example + 1 transition, 150-220 words. "Argument 2" follows the same structure. "Argument 3" is REQUIRED — claim sentence + 2-3 evidence sentences + 1 example + 1 transition, 150-220 words (same structure as Arguments 1 and 2). Three arguments is the minimum for a credible op-ed at this length floor (1500w minimum per v1.5.216.62.67 — raised from 1100w in v62.52 to give each Argument N section ~150-220 words of substantive evidence rather than cramped 110-word abstracts). "The Objection" steelmans the opposing view in one paragraph, then refutes with specifics — 100-180 words total. "What This Means" covers implications and stakes for the reader — 100-150 words. "FAQ" is 3-5 Q&A addressing common objections and "what should I do next" — 40-60 words per answer, answer-first format. "Conclusion and Call to Action" restates the thesis reframed by the evidence, one sentence of stakes, then a concrete CTA. TONE: First person is ENCOURAGED ("I argue", "I have seen", "In my view") but avoid weak hedges ("I think", "I feel", "I believe"). Lead with a specific anecdote, a surprising stat with source, or a provocative one-line claim — never "In today\'s world". Strongest argument first, weakest never. Steelman the counterargument before refuting. Every factual claim needs an outbound citation — aim for 4-8 pool-matched links per 1000 words with descriptive noun-phrase anchor text (not "click here" or naked URLs). Qualified claims beat absolutism ("in most cases" beats "always"). Label the piece as opinion somewhere in the prose. End with a concrete call to action (share, contact, read the cited source, act) — not "let me know what you think". NO INLINE BOLD anywhere in the body — pull-quote sentences should stand on their own without ** markup; the formatter will style them via CSS if pull-quote markup is added later.', 'schema' => 'OpinionNewsArticle'],
            ...
            'review' => ['sections' => 'Key Takeaways, What It Is and Who It Is For, Key Specs and Features, Hands-on Experience, Pros and Cons, Verdict and Rating, FAQ, References', 'guidance' => 'Honest product evaluation. Evidence-based claims. Include specific measurements and comparisons. Declare a clear verdict. v1.5.216.62.74 NUMERIC RATING RULE: in the "Verdict and Rating" H2 section, the FIRST sentence MUST contain a numeric rating in the format "X/10" or "X/5" or "X.X/5" — examples: "Verdict: 8.5/10." or "Rating: 4.5/5." or "We rate the [product] 9/10 overall." This is mandatory because the schema generator extracts reviewRating from this exact pattern; without a numeric rating in this section, the JSON-LD Review will ship without reviewRating which Google deprioritizes for rich-result eligibility. Use a real rating that matches the verdict prose (don\'t write "8/10" then hedge "but really we wouldn\'t recommend it"). Acceptable scales: X/5 (5 = perfect), X/10 (10 = perfect). Decimals OK (4.5/5, 8.5/10). NEVER omit the numeric rating in this section.', 'schema' => 'Review'],
            ...
            'buying_guide' => ['sections' => 'Key Takeaways, Quick Picks Table (5-7 products with one-line summary + price tier), 5-7 Individual Product Mini-Reviews (EACH product gets its OWN H2 heading numbered like "1. Product Name", "2. Product Name" etc with 250-350 words covering hands-on impressions + key features + pros + cons + recommended buyer + verdict), Pros and Cons of Each Top Pick (consolidated bullet list), What to Look For When Buying (buying criteria — 250-350 words), FAQ, References', 'guidance' => 'BUYING GUIDE FORMAT: You MUST create EXACTLY 5-7 products in the lineup. Each product gets its OWN H2 heading formatted as "1. Product Name", "2. Product Name" etc. — NEVER collapse multiple products under a single "Individual Reviews" H2. Write 250-350 words per product mini-review covering: hands-on impressions, key features, 3 pros, 2 cons, who it is best for, and a one-line verdict. The "What to Look For When Buying" section needs 250-350 words on buying criteria. Total target: 2000-3000 words. Start with a Quick Picks summary table comparing all 5-7 products on price + standout feature + best for. End with buying criteria covering features that matter and red flags to avoid.', 'schema' => 'Article'],
            ...
            // v1.5.216.62.47 — §3.1A compliance: interview is "Intro → Bio
            // → Q&A pairs ARE the content → Closing. NO separate FAQ (the
            // whole piece is Q&A). Keeps References." Pre-fix had Key
            // Takeaways AND a separate FAQ section — the FAQ duplicated
            // the body's Q&A structure and the Key Takeaways doesn't
            ...
            // v1.5.216.62.47 — §3.1A compliance: live_blog is "Timestamped
            // updates in reverse-chronological order. Coverage intro at
            // top, then `HH:MM — [update]` entries. NO Key Takeaways, FAQ,
            // or static References." Pre-fix had FAQ + References — FAQ
            // doesn't fit timestamped event coverage, References should
            // be inline within update entries (not a static section).
            'live_blog' => ['sections' => 'What We Are Covering, Timestamped Updates (latest first, format: HH:MM — [update text])', 'guidance' => 'Real-time event coverage in reverse-chronological order. Coverage intro paragraph at the top, then a stream of timestamped updates (latest first). Each update is short and punchy with a clock-time prefix (e.g. "14:32 — Markets reacted within seconds…"). Include quotes from participants inline. Do NOT add Key Takeaways, FAQ, or a static References H2 — live coverage cites sources inline within the update entries.', 'schema' => 'LiveBlogPosting'],
            'press_release' => ['sections' => 'The Announcement, Key Highlights, About the Company, Media Contact, FAQ, References', 'guidance' => 'Corporate announcement aiming for media pickup. Target 400 words total, hard ceiling 500. Formal third person throughout except inside quotes. SECTION PURPOSE: the article title is already the H1 headline — do NOT emit another "Headline" or "Subheadline" H2. "The Announcement" is the main news block — start with `CITY, STATE — Month DD, YYYY — [Subject] [active verb] [what] [where] [why].` as the first paragraph (dateline + lede answering the 5 Ws in the first 25 words), then 3-4 flowing body paragraphs (2-3 sentences each, inverted pyramid — most important facts first, context after), with 1-2 executive quotes embedded INLINE inside the body as blockquotes (format: `> "Quote sentence," said Jane Doe, CTO at Acme Inc.`) — each quote 1-2 sentences, no cliché CEO-speak. Total 225-275 words under this H2. "Key Highlights" is a 3-5 item bullet list with named stats or numbers for AI snippet extraction (40-60 words). "About the Company" is 60-100 word boilerplate — what the company does, where based, since when. "Media Contact" is 20-30 words — name, title, email, phone. "FAQ" is 2-3 Q&A anticipating journalist questions (80-120 words total), 30-40 words per answer, answer-first. NEVER emit an H2 called "Dateline and Lede" or "Body" or "Quote" or "Subheadline" — those are prose structures inside "The Announcement", not standalone headings. HEADLINE RULES: the title itself must be under 70 characters with an active verb (launches, introduces, reveals, reports, announces), NOT "unveils" / "unleashes" / "disrupts". BAN LIST — these words make journalists auto-reject and must not appear anywhere in the article: groundbreaking, disruptor, disruptive, revolutionary, game-changing, industry-leading, one-stop-shop, unique, innovative, breaking, urgent, exclusive, best-in-class, cutting-edge, next-generation, world-class, leverage, synergy, unleash, unveil. Use active verbs and specific numbers instead. LINK RULES: maximum 1-2 outbound links per release (Google post-2025 treats excess as spam), no exact-match keyword anchor text. NO clever wordplay — direct, factual, newsworthy.', 'schema' => 'NewsArticle'],
            'personal_essay' => ['sections' => 'Opening Scene, The Central Event, Scenes and Sensory Detail, Reflection, Resolution or Lesson', 'guidance' => 'First-person literary essay, 1200-2500 words (target 1500 — the NYT Modern Love standard). Every section below targets specific word counts summing to this. SECTION PURPOSE: "Opening Scene" opens IN MEDIA RES — the first sentence must be action, dialogue, or a specific concrete observation, NEVER "Growing up, I always…" / "For as long as I can remember…" / "We all know…". Drop the reader into a moment (200-300 words). "The Central Event" is the one thing this essay turns on — the fulcrum around which every scene orbits (300-400 words). "Scenes and Sensory Detail" builds the story through concrete scenes with THREE sensory data points per moment (e.g. "small, dusty, faintly smelling of mothballs"), attributed dialogue, named places, named people, specific dates or years — not "a café" but "the Paragon Café on Lonsdale Street, October 2019" (400-600 words). "Reflection" is what the narrator understands NOW that they did not understand THEN — earned insight, not pronounced (150-250 words). "Resolution or Lesson" is the last beat — a single concrete image or line that lands the transformation (100-200 words). CRAFT RULES: Write in first person throughout. Show, do not tell. Every scene must have transformation — without change it is a travel diary, not an essay. BAN LIST — these generic openings / phrasings make the essay sound AI-written and must never appear: "Growing up", "For as long as I can remember", "All my life", "We all know", "In today\'s world", "Generally speaking", "Most people think", "Looking back now I realize" (as a cliché opener), "at the end of the day". BAN LIST — vague placeholders that undercut Experience signals: "a café", "a friend", "a while ago", "someone said", "a place I went", "many years ago". Replace every vague marker with a specific name, date, or place. TONE: Honest, specific, sometimes uncomfortable. Earned insight over pronounced moral — NEVER end with "And that\'s when I learned that [life lesson]". Let the image carry the meaning. NO INLINE BOLD anywhere in the body — pull-quote sentences stand on their own without ** markup; the formatter handles styling via CSS.', 'schema' => 'BlogPosting'],
        ...
        $shared = ' CRITICAL RULES FOR ALL TYPES:'
            ...
            // v1.5.216.62.53 — Bolding rule (universal across all 21 types,
            // all languages). Per SEO-GEO-AI-GUIDELINES.md §4B "Random
            // bolding" row, inline emphasis bolding reads as AI-generated.
            // The only allowed bolds are list section labels at start of
            // line ending with a colon (**Pros:**, **Cons:**, etc.) — those
            // are structural anchors the formatter uses to auto-style boxes.
            . ' BOLDING: Do NOT use bold (** in markdown or <strong>) for emphasis ANYWHERE in the article body. No bolding terms, definitions, focus keywords, topic sentences, or pull quotes. The ONLY allowed bold is a list section label at start of a line ending with a colon: **Pros:**, **Cons:**, **Prerequisites:**, **What You Will Need:**. Bolding random text is a tell that content is AI-generated.'
            // E-E-A-T
    ...
    private static function generate_section( string $keyword, string $heading, int $index, array $options, array $secondary, array $lsi, string $system, string $trends, string $intent = 'informational', array $places_pool = [], string $places_location = '' ): string {
        ...
        if ( $is_takeaways ) {
            ...
            $intro_rule = '';
            if ( $index === 1 ) {
                // v1.5.216.62.53 — removed "Bold the keyword once: **{$keyword}**".
                // Random keyword bolding in body prose is a §4B AI-tell signal
                // (see SEO-GEO-AI-GUIDELINES.md §4B "Random bolding" row). SEO
                // plugins check for keyword PRESENCE in the intro paragraph,
                // not for bold formatting around it — plain-text mention
                // satisfies the SEO check without the AI-generated tell.
                $intro_rule = "\n\nINTRODUCTION RULE (SEO PLUGINS CHECK THIS PARAGRAPH):\n- The FIRST SENTENCE of this section must contain the exact phrase \"{$keyword}\" naturally\n- SEO plugins (AIOSEO, Yoast, RankMath) check this paragraph for the focus keyword presence (plain text — do NOT bold it)\n- Write the intro like a human opening a conversation — not a definition or press release\n- Do NOT start with '[Keyword] is...' or '[Keyword] refers to...' — those are AI patterns\n- Jump into a specific fact, opinion, or context that includes the keyword naturally";
            }
    ...
    private static function enforce_geo_requirements( string $markdown, string $keyword, array $job ): string {
        ...
        //
        // v1.5.199 — gated by content_type. A Quick Comparison table makes sense
        // for types that structurally compare options or products: listicle,
        // how_to, buying_guide, comparison, review, ultimate_guide, blog_post,
        // tech_article. It is INAPPROPRIATE (and was producing embarrassing
        // auto-injected tables) on: press_release, opinion, news_article,
        // recipe, faq_page, interview, personal_essay, live_blog, sponsored,
        // case_study, glossary, white_paper, scholarly_article. Matches the
        // v1.5.60 allowlist used by the outline-phase table-enforcement prompt.
        $table_compatible = [ 'listicle', 'how_to', 'buying_guide', 'comparison', 'review', 'ultimate_guide', 'pillar_guide', 'blog_post', 'tech_article' ];
        $table_enforce_ok = in_array( $content_type, $table_compatible, true );
        $has_table = (bool) preg_match( '/\|.+\|.+\|/', $markdown );
        if ( $table_enforce_ok && ! $has_table ) {
            $table = '';
    ...
    private static function assemble_final( array $job ): array {
        ...
        // v1.5.216.62.53 — Server-side inline-bold strip. Even with the
        // prompt-level BOLDING rule (in $shared rules block), the AI
        // statistically still bolds terms / definitions / topic sentences
        // / pull-quote intros. User-reported on the Opinion article retest:
        // 5 bolded fragments in body including "A large language model (LLM)
        // is a neural network..." (random definition emphasis) and "Volume
        // is the enemy..." (topic-sentence bolding) — both classic AI tells
        // per SEO-GEO-AI-GUIDELINES.md §4B "Random bolding" row.
        //
        // Strategy: walk every <strong>...</strong> in the body. If the
        // tag's content matches a structural label pattern (label at start
        // of line ending with a colon, e.g. "Pros:", "Cons:",
        // "Prerequisites:", "What You Will Need:"), keep it — those are
        // anchors the Pros/Cons / What-You-Need formatter uses to auto-
        // style colored boxes. Otherwise unwrap the strong tag, leaving
        // the inner text in place.
        //
        // Universal across all 21 content types and all 38+ languages.
        // The label pattern is language-agnostic (it's a strict structural
        // shape, not a word match), and even when the article is non-
        // English the **Pros:** / **Cons:** / etc. structural anchors
        // come through translated by canonical_translation_block but
        // still fit the pattern (e.g. **Pros:** stays as ** + label + : **
        // because v1.5.x localizes the label content but keeps the
        // structural form).
        $html = preg_replace_callback(
            '#<strong\b[^>]*>(.*?)</strong>#is',
            function ( $m ) {
                $inner_text = trim( wp_strip_all_tags( $m[1] ) );
                // Keep tag if it's a structural label: short (≤32 chars)
                // and ends with a colon. Examples: "Pros:", "Cons:",
                // "Prerequisites:", "What You Will Need:", "메모:".
                if ( $inner_text !== '' && mb_strlen( $inner_text ) <= 32 && substr( $inner_text, -1 ) === ':' ) {
                    return $m[0];
                }
                // Otherwise unwrap — keep inner content, drop <strong>.
                return $m[1];
            },
            $html
        ) ?: $html;
...
- For ONE key insight per major section, write a specific factual sentence as a single bold line on its own paragraph. Example: **Dogs fed raw diets showed 40% fewer vet visits in the first year.** The bold sentence MUST contain a real fact or number from the article — NEVER write generic placeholder text. (1 per H2 section, max 2 per article)
