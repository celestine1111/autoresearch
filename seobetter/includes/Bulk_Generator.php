                    ...
                    if ( $post_id && ! is_wp_error( $post_id ) ) {
                        ...
                        try {
                            ...
                            if ( $meta_desc !== '' ) {
                                ...
                                // v1.5.216.62.87 — Re-run Schema_Generator AFTER meta is stored so
                                // the JSON-LD BlogPosting.description picks up the AI-generated
                                // _seobetter_meta_description (via Schema_Generator::get_post_description).
                                // Pre-fix the schema was generated by save_post -> analyze_on_save
                                // BEFORE meta_description was stored, leaving JSON-LD with the
                                // build_clean_description fallback ("This table highlights it...").
                                // Single Generate avoids this because rest_save_draft regenerates
                                // schema after sync_seo_plugin_meta (seobetter.php:2454).
                                $schema_gen = new Schema_Generator();
                                $post_obj = get_post( $post_id );
                                $schema_array = $post_obj ? $schema_gen->generate( $post_obj ) : [];
